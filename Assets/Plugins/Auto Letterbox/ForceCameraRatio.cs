...
namespace AutoLetterbox
{
    ...
    public class ForceCameraRatio : MonoBehaviour
    {
        ...
        void StartStart(Scene scene, LoadSceneMode mode)
        {
            Start();
        }
        ...
        private void Awake()
        {
            SceneManager.sceneLoaded += StartStart;
        }
        ...
        private CameraRatio GetCameraRatioByCamera(Camera _camera)
        {
            ...
            for (int i = 0; i < cameras.Count; i++)
                if (cameras[i] != null && cameras[i].camera == _camera) return cameras[i];
        ...
        {
            for (int i = cameras.Count - 1; i >= 0; i--) if (cameras[i].camera == null) cameras.RemoveAt(i);
        }
