...
{
    public class FScale_QuickConverterWindow : EditorWindow
    {
        ...
        {
            FScale_QuickConverterWindow window = (FScale_QuickConverterWindow)GetWindow(typeof(FScale_QuickConverterWindow));

            window.minSize = new Vector2(250f, 225f);

            window.titleContent = new GUIContent("PNG Conversion");
            window.Show();
        }
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Duplicate and convert Files (" + textures.Count + ")"))
            {
                ...
                {
                    Texture2D pngDuplicate = FScale_EditorToolsMethods.DuplicateAsPNG(textures[i]);
        ...
        private static void ConvertMaterialTexturesToPNGs(MenuCommand menuCommand)
        {
            ...
            if (targetMaterial)
            {
                ...
                for (int i = 0; i < textures.Count; i++)
                {
                    ...
                    {
                        Texture2D duplicated = FScale_EditorToolsMethods.DuplicateAsPNG(textures[i].tex);
                        if (duplicated != null)
