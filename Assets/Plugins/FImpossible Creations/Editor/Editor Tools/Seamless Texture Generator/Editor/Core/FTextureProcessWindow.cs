...
namespace FIMSpace.FSeamlessGenerator
{
    public abstract class FTextureProcessWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(0, 0, position.width, 44), titleContent, FGUI_Resources.HeaderStyleBig);
            GUI.Label(new Rect(0, 19, position.width, 50), titleContent.tooltip, FGUI_Resources.HeaderStyle);
            int currY = 60;

            GUILayout.Space(currY + 4);

            previewSize = EditorGUILayout.IntSlider(new GUIContent("Preview Size"), previewSize, 64, 256);
            GUILayout.Space(2);
            ...
            if (drawPreviewScale)
                previewScale = (FEPreview)EditorGUILayout.EnumPopup("Preview Scale", previewScale);
            else
                currY -= 18;
            ...
            if (somethingChanged) RefreshSeamlessPreview();
            ...
            GUI.Label(new Rect(0, currY - 8, position.width / 2, 48), "Source", FGUI_Resources.HeaderStyle);
            GUI.Label(new Rect(position.width / 2, currY - 8, position.width / 2, 48), previewTitle, FGUI_Resources.HeaderStyle);
            currY += 48;
        ...
        protected void OnGUIFooter()
        {
            ...
            if (drawApplyButton)
                if (GUILayout.Button("APPLY TO TEXTURE (No Undo)", new GUILayoutOption[1] { GUILayout.Height(38) }))
                {
            ...
            if (drawPreviewButton)
                if (GUILayout.Button("PREVIEW ON SCENE (Undo after 'reimport' on texture file)\nTexture can look too sharp (mip maps disabled for preview)", new GUILayoutOption[1] { GUILayout.Height(38) }))
                {
                    for (int i = 0; i < textures.Count; i++)
                    {
            ...
            if (drawGenCopyButton)
                if (GUILayout.Button("GENERATE BACKUP AND APPLY", new GUILayoutOption[1] { GUILayout.Height(38) }))
                {
                    for (int i = 0; i < textures.Count; i++)
                    {
        ...
        private void RefreshSeamlessPreview()
        {
